...
genderWeight <- function(ratio=cueweighting, normalize=normalizeWeights){    
  ifelse(normalize, 1/(ratio+1)*2, 1)
}

caseWeight <- function(ratio=cueweighting, normalize=normalizeWeights){   
  ifelse(normalize, 1/(ratio+1)*2, 1)
}
...
# Modeling syncretic case:
#
# No syntactic cue, full match for syncretic cases
#     target_match = list(c(1,1), c(1,1), c(1,1), c(0,1), c(0,1), c(0,1)),
# distractor_match = list(c(1,1), c(0,0), c(0,1), c(0,1), c(1,0), c(1,1)),
#
# No syntactic cue, half-match for syncretic cases
#     target_match = list(c(1,1), c(1,1), c(1,1), c(0,1), c(0,1), c(0,1)),
# distractor_match = list(c(1,1/2), c(0,0), c(0,1/2), c(0,1/2), c(1,0), c(1,1/2)),
...
distortion <- function(perc){
  ...
  if(I_perc==0){
    model_6cond <- list(
      # the first number is gender match/mismatch between the noun and the verb retrieval cues
      # the second number is case match/mismatch between the noun and the verb retrieval cues
    target_match = list(c(1,1), c(1,1), c(1,1), c(0,1), c(0,1), c(0,1)),
distractor_match = list(c(1,1), c(0,0), c(0,1), c(0,1), c(1,0), c(1,1)),
      ...
      Distractor = c("Gender match, Case match (partial)",     # a  Náklad na parník překvapivě byl
                     ...
                     "Gender mismatch, Case mismatch",         # d *Náklad na parník překvapivě byl-a
                     "Gender match, Case mismatch",            # e *Náklad na střech-u překvapivě byl-a
                     "Gender match, Case mismatch"),           # f *Náklad na loď překvapivě byl-a
