...
strWeight <- function(ratio=cueweighting, normalize=normalizeWeights){   # CHANGED for 3 CUES
  ifelse(normalize, ratio/(2+ratio)*3, ratio)
}
semWeight <- function(ratio=cueweighting, normalize=normalizeWeights){    # CHANGED for 3 CUES
  ifelse(normalize, 3/(ratio+2), 1)
}

numWeight <- function(ratio=cueweighting, normalize=normalizeWeights){   # CHANGED for 3 CUES
  ifelse(normalize, 3/(ratio+2), 1)
}

genderWeight <- function(ratio=cueweighting, normalize=normalizeWeights){    # CHANGED for 3 CUES
  ifelse(normalize, 3/(ratio+2), 1)
}

caseWeight <- function(ratio=cueweighting, normalize=normalizeWeights){   # CHANGED for 3 CUES
  ifelse(normalize, 3/(ratio+2), 1)
}
...
compute_cond_means <- function(results){
	...
	condMeans <- results %>% group_by(Set, Condition, Target, Distractor) %>% 
	  summarise(
...
###############################################################
## Standard interference model with 6 conditions
###############################################################
# percolation with probability, perc

# How do we model syncretic case?
#
#
# - As full match for both subject and attractor:
# target_match = list(c(1,1,1), c(1,1,1), c(1,1,1), c(1,0,1), c(1,0,1), c(1,0,1)),
# distractor_match = list(c(0,1,1), c(0,0,0), c(0,0,1), c(0,0,1), c(0,1,0), c(0,1,1)),
#
#  - As half match for attractor:
# target_match = list(c(1,1,  1), c(1,1,1), c(1,1,  1), c(1,0,  1), c(1,0,1), c(1,0,  1)),
# distractor_match = list(c(0,1,1/2), c(0,0,0), c(0,0,1/2), c(0,0,1/2), c(0,1,0), c(0,1,1/2)),
...
distortion <- function(perc){
  ...
  if(I_perc==0){
    model_6cond <- list(
      # the first number in the triplet is syntactic match: 
      # always a match for target, and a mismatch for distractor
      # the second number is gender match/mismatch between the noun and the verb retrieval cues
      # the third number is case match/mismatch between the noun and the verb retrieval cues
     target_match = list(c(1,1,1), c(1,1,1), c(1,1,1), c(1,0,1), c(1,0,1), c(1,0,1)),
 distractor_match = list(c(0,1,1), c(0,0,0), c(0,0,1), c(0,0,1), c(0,1,0), c(0,1,1)),
  Target = c("Gender match, Case match (partial)",     # a Náklad na parník překvapivě byl
             "Gender match, Case match (partial)",     # b Náklad na střech-u překvapivě byl 
             "Gender match, Case match (partial)",     # c Náklad na loď překvapivě byl 
             "Gender mismatch, Case match (partial)",  # d *Náklad na parník překvapivě byl-a
             "Gender mismatch, Case match (partial)",  # e *Náklad na střech-u překvapivě byl-a
             "Gender mismatch, Case match (partial)"), # f *Náklad na loď překvapivě byl-a
  Distractor = c("Gender match, Case match (partial)",     # a Náklad na parník překvapivě byl
                 "Gender mismatch, Case mismatch",         # b Náklad na střech-u překvapivě byl 
                 "Gender mismatch, Case match (partial)",  # c Náklad na loď překvapivě byl 
                 "Gender mismatch, Case match (partial)",  # d *Náklad na parník překvapivě byl-a
                 "Gender match, Case mismatch",            # e *Náklad na střech-u překvapivě byl-a
                 "Gender match, Case match (partial)"),    # f *Náklad na loď překvapivě byl-a
      weights = list(c(strWeight(), genderWeight(), caseWeight()))
    )
  }else{
    model_6cond <- list(
      # the first number in the triplet is syntactic match: 
...
## PARAMETER MATRIX
##
create_param_matrix <- function(perc,model,iterations=1000,verbose=FALSE){
...
# d$chunk <- rep(chunknames,total/n_chunks)
	...
	for(i in 1:nrow(d)){
	  ...
	  d$weights3[i] <- semWeight(d$cueweighting[i])
	}
	return(d)
}
...
run <- function(d){
	...
	weights <- select(d, weights1, weights2, weights3) %>% as.matrix()
	#
...
## ACT-R
###############################################################
activation <- function(Pij=matrix(c(1,1,1),1,3,T), prom=0, 
                       weights=matrix(c(1,1,1),1,3,T), 
                       times=lp, b=blc, noise=act_r_noise(ans), 
                       W=ga, dec=bll, S=mas, P=mp, C1=pic, C2=psc){
...
spreading_act <- function(Pij=matrix(c(1,1,1),1,3,T), S=mas){
  Sji <- S+log(Pij)
  Sji <- ifelse(Sji==-Inf | is.na(Sji), 0, Sji)
  ifelse(Sji < 0, print("!!! WARNING: Sji < 0 !!!"), T)
...
weight_spreading_act <- function(Sji=matrix(c(0,0,0),1,3,T), weights=matrix(c(1,1,1),1,3,T), W=ga){
  Wkj <- W*weights/rowSums(weights)
  rowSums(Wkj*Sji)
}

mismatch_penalty <- function(cuematch=matrix(c(1,1,1),1,3,T), P=mp){
  penalty <- cuematch-1
...
match_quality <- function(match=matrix(c(1,1,1),1,3,T), sim=cuesim, blact=0, tau=rth, s=ans, q=qcf, q2=qco){
  qcorr <- 1/(1+q*exp(-(blact-tau-q2)))
  conf <- match[,c(3,2,1)]*(1+sim) 
  (match+conf) * qcorr
}
