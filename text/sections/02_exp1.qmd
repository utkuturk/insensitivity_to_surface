...
exp1_verbal_fit_path <- "../../utility/models/m.exp1.verbal.rds"
...
To make this pattern directly interpretable at the condition level, we then report model-implied marginal attraction contrasts. These summaries mirror the same conclusion: strong negative attraction effects for nominal conditions (`r get_value(exp1_atts, text, contrast == "Gen-Current")`; `r get_value(exp1_atts, text, contrast == "Gen-TL24")`), no attraction for verbal conditions (`r get_value(exp1_atts, text, contrast == "RC")`), and no clear Gen-Current vs Gen-TL24 difference in attraction (`r get_value(exp1_atts, text, contrast == "GenCurrent - GenTL24")`). We also observe lower overall acceptability in the current nominal dataset relative to @TurkLogacev2024 (`r get_value(overall_df, text)`), indicating a distributional shift in baseline acceptability without a corresponding shift in attraction magnitude.
...
To provide formal evidence for the null-effect claims, we computed Bayes Factors with bridge sampling (marginal-likelihood model comparison). We compared nested models so that BF~01~ > 1 favors the simpler (null) model.
...
bridge_recompile_exp1 <- FALSE

format_bf <- function(bf) {
    ifelse(
        is.na(bf),
        "NA",
        ifelse(bf > 100, ">100", ifelse(bf < 0.01, "<0.01", sprintf("%.2f", bf)))
    )
}
...
safe_bridge <- function(
    ...
    out <- tryCatch(
        bridgesampling::bridge_sampler(fit, silent = TRUE, recompile = recompile),
        error = function(e) stop(sprintf("bridge_sampler failed: %s", e$message))
    )
...
missing_models <- c(
    if (is.null(fit_full)) "full (m.exp1.full.prior_n10.rds or m.exp1.all.yes.rds)",
...
if (file.exists(bf_cache_path_exp1)) {
    ...
    dir.create(dirname(bf_cache_path_exp1), recursive = TRUE, showWarnings = FALSE)
...
With bridge sampling, BF~01~ = `r bf01_3way_txt` for the three-way attraction contrast (full model vs no-three-way model), and BF~01~ = `r bf01_2way_txt` for overall attraction in the no-three-way family (no-three-way model vs no-(G x A) model). For the primary contrast, this corresponds to `r bf_3way_interp`.
...
#| tbl-cap: "Prior-sensitivity analysis for Bayes factors in Experiment 1. BF~01~ > 1 favors the null."
#| warning: false
#| message: false
...
compute_prior_sens_exp1 <- function(grid, rope_halfwidth = 0.20) {
    purrr::pmap_dfr(
        grid,
        function(Prior_Set, Fixed_Effect_Prior, fixed_sd, full_obj, full_rds, no3way_obj, no3way_rds, noGA_obj, noGA_rds) {
            ...
            if (is.null(fit_full_i) || is.null(fit_no3way_i) || is.null(fit_noGA_i)) {
                stop(
                    paste0(
                        "Missing prior-sensitivity models for ", Prior_Set, ": ",
                        if (is.null(fit_full_i)) full_rds else "",
                        if (is.null(fit_full_i) && (is.null(fit_no3way_i) || is.null(fit_noGA_i))) "; " else "",
                        if (is.null(fit_no3way_i)) no3way_rds else "",
                        if (is.null(fit_no3way_i) && is.null(fit_noGA_i)) "; " else "",
...
if (file.exists(bf_sens_cache_path_exp1)) {
    ...
    bf_prior_sens_exp1 <- compute_prior_sens_exp1(prior_grid_exp1, rope_halfwidth_exp1)
    dir.create(dirname(bf_sens_cache_path_exp1), recursive = TRUE, showWarnings = FALSE)
...
bf_prior_sens_exp1_print <- bf_prior_sens_exp1 %>%
    ...
    dplyr::rename(
        ...
        `BF01 (G x A x T1)` = BF01_GxAxT1,
        `BF01 (G x A)` = BF01_GxA,
        `Interpretation (G x A x T1)` = BF01_GxAxT1_Interpretation
    )
...
- Experiment 1 tested the effects of plural number marking in speeded acceptability judgments of sentences with nominal and verbal modifiers in Turkish.
- We were able to replicate the canonical attraction pattern for nominal attractors, with plural-marked nominal attractors [@TurkLogacev2024; @LagoEtAl2019]. 
- On the other hand, experiment 1 found no evidence that phonological overlap between nominal and verbal plural morphemes in Turkish induces attraction. 
- Participants reliably rejected ungrammatical sentences with plural-marked verbal attractors, contrasting with the canonical attraction effects observed for nominal attractors. 
- We verified that the negative three way interaction (Grammaticality x Attractor Number x RC vs Nominal) was not an artifact of the mixed design by fitting a model restricted to verbal conditions, which showed no evidence for an interaction between grammaticality and number on the attractor.
- We also verified that the null effect for verbal attractors was not due to a lack of power by computing Bayes Factors with bridge sampling, which provided moderate evidence for the null hypothesis of no verbal attraction. This conclusion was robust across a range of reasonable fixed-effect priors.

- Our aim was to test whether an attractor with phonological overlap to possible subjects with necessary features would induce attraction effects. 
- The account of cue-based theory proposed by @Slioussar2018 would predict that the Verbal RC would also activate features of the subjecthood, nominative marking, and plural number due to string ambiguity, and thus induce attraction. 
- On the other hand, the statistical association account of @LagoEtAl2019 would predict that only nominal attractors would induce attraction, since they are more strongly associated with the subjecthood and plural features in the input.
- And The fact that Verbal RCs cannot act as controllers, they would not have increased association with subjecthood, thus would not induce attraction.
- Our results support the statistical association account, as we observed strong attraction effects for nominal attractors but no evidence for attraction from verbal attractors, despite their phonological overlap. 

- In our experiment, we included nominal attractors for a better direct comparison to prior work and to ensure that participants were exposed to robust attraction items. 
- Our results and between-experiment comparisons indicate that overall acceptability for nominal attractor sentences was lower than in @TurkLogacev2024.
- This aligns with prior work showing that trial distributions modulate judgments. While previous studies drove this effect via instructions or fillers [@HammerlyEtAl2019;@ArehalliWittenberg2021], we demonstrate that experimental conditions and the presence of an effect in a condition subset also modulate overall acceptability. 
- Despite this shift in acceptability, the within-experiment statistics---specifically, exposure to verbal attraction items---did not substantially reduce attraction magnitude within nominal items.
- This is unexpected given that the previous studies showed that the distribution of no attraction-inducing items might affect participants a priori bias, thus changing the overall attraction pattern [@HammerlyEtAl2019;@Turk2022]. 
- It seems that high number of fillers prevented participants from adjusting their a priori bias towards 'yes' responses. 
- The number of no attraction-inducing items (verbal) did not change the overall distribution of items enough to shift the bias, thus not changing the attraction pattern in nominal items.

- A potential concern is that our mixed design---combining canonical nominal attractors with verbal ones---influenced response patterns. 
- The presence of robust nominal attraction may have altered participant strategies, potentially masking weaker verbal effects or making participants directly compare verbal attractor conditions to nominal ones. 
- To determine if the absence of verbal attraction in Experiment 1 was genuine rather than a distributional artifact, Experiment 2 removed all nominal attractors. This design tests whether the null effect persists when verbal morphology is the sole potential source of interference.
