...
```{r}
#| label: exp1-verbal-bayes-factor
#| message: false
#| warning: false

# Cache behavior:
# - If bf_exp1_verbal_bridge.rds exists, it is loaded.
# - If it does not exist, set bf_compute_if_missing_exp1_verbal <- TRUE once.
bf_cache_path_exp1_verbal <- "../../utility/models/bf_exp1_verbal_bridge.rds"
bridge_recompile_exp1_verbal <- FALSE
bf_force_recompute_exp1_verbal <- FALSE
bf_compute_if_missing_exp1_verbal <- TRUE
...
safe_read_fit <- function(rds_path) {
...
verbal_full_fit_path <- "../../utility/models/m.exp1.verbal.rds"
verbal_noGA_fit_path <- "../../utility/models/m.exp1.verbal.noGA.rds"

fit_verbal_full_bf <- if (exists("m.exp1.verbal")) {
    m.exp1.verbal
} else {
    safe_read_fit(verbal_full_fit_path)
}
fit_verbal_noGA_bf <- safe_read_fit(verbal_noGA_fit_path)

if (file.exists(bf_cache_path_exp1_verbal) && !bf_force_recompute_exp1_verbal) {
    bf_exp1_verbal <- readRDS(bf_cache_path_exp1_verbal)
} else if (
    !is.null(fit_verbal_full_bf) &&
        !is.null(fit_verbal_noGA_bf) &&
        (bf_force_recompute_exp1_verbal || bf_compute_if_missing_exp1_verbal)
) {
    bridge_full_verbal <- safe_bridge(
        fit_verbal_full_bf,
        "../../utility/models/m.exp1.verbal.bridge.rds",
        recompile = bridge_recompile_exp1_verbal
    )
    bridge_noGA_verbal <- safe_bridge(
        fit_verbal_noGA_bf,
        "../../utility/models/m.exp1.verbal.noGA.bridge.rds",
        recompile = bridge_recompile_exp1_verbal
    )

    bf_exp1_verbal <- list(
        bf01 = compute_bf01(bridge_full_verbal, bridge_noGA_verbal),
        comparison = "RC-only full vs RC-only no-(GxA)"
    )
    dir.create(
        dirname(bf_cache_path_exp1_verbal),
        recursive = TRUE,
        showWarnings = FALSE
    )
    saveRDS(bf_exp1_verbal, bf_cache_path_exp1_verbal)
} else {
    bf_exp1_verbal <- list(
        bf01 = NA_real_,
        comparison = "RC-only full vs RC-only no-(GxA)"
...
bf01_verbal <- as.numeric(bf_exp1_verbal$bf01)
bf01_verbal_txt <- format_bf(bf01_verbal)
bf_verbal_interp <- interpret_bf(bf01_verbal)

if (is.na(bf01_verbal)) {
    bf_verbal_sentence <- paste(
        "This BF has not yet been computed.",
        "Fit ../../utility/models/m.exp1.verbal.noGA.rds and then either set",
        "bf_compute_if_missing_exp1_verbal <- TRUE once or place",
        "bf_exp1_verbal_bridge.rds in utility/models."
    )
} else {
    bf_verbal_sentence <- paste0(
        "We found BF~01~ = ",
        bf01_verbal_txt,
        ", indicating ",
        bf_verbal_interp,
        " for the RC-only null model without G x A."
    )
}
```

This BF targets the verbal null directly. We compare the RC-only model with a Grammaticality x Attractor Number interaction against an otherwise matched RC-only model without that interaction. Here the null hypothesis is that there is no attraction in RC conditions (G x A = 0), so BF~01~ > 1 favors the null model and BF~01~ < 1 favors the model with verbal attraction.

`r bf_verbal_sentence`
...
<!-- CHECK THIS: Para 1 is dense but okay stylistically. "The answer is no" is punchy and human. The sentence starting "We confirmed that this dissociation..." is long but natural. Main AI tell: the semicolon before "we return to this" — reads like an AI transition. Consider just ending the paragraph. -->
Experiment 1 tested whether phonological overlap between nominal and verbal reflexes of plural marking in Turkish is sufficient to induce agreement attraction. The answer is no. Participants reliably rejected ungrammatical sentences with plural-marked verbal attractors, while showing canonical attraction in the nominal-attractor conditions that replicate prior work [@TurkLogacev2024; @LagoEtAl2019]. We confirmed that this dissociation is not an artifact of the mixed design: a follow-up model restricted to verbal-attractor trials showed no Grammaticality × Attractor Number interaction. This pattern is more consistent with accounts in which attraction depends on controller-relevant cues rather than surface-form overlap alone [@LagoEtAl2019]; we return to this in the General Discussion.

<!-- CHECK THIS: Para 2 is mostly fine. "A potential concern" opener is slightly AI-formulaic but common in academic writing. The parenthetical "(we discuss the implications...)" is a bit AI — consider moving the forward reference to its own sentence or cutting it. -->
A potential concern is that the mixed design---combining canonical nominal attractors with verbal ones---may have influenced response patterns in ways that obscure genuine verbal attraction. The presence of robust nominal-attractor items could have shifted participant strategies, either masking weaker verbal effects or encouraging direct comparison between the two attractor types. Moreover, overall acceptability for nominal-attractor items was lower in the present study than in @TurkLogacev2024, suggesting that the experimental context modulated response patterns (we discuss the implications of this distributional shift further in the General Discussion). To determine whether the absence of verbal attraction is genuine or a distributional artifact of the mixed design, Experiment 2 removed all nominal attractors, testing whether the null effect persists when verbal morphology is the sole potential source of interference.
