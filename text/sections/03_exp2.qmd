...
exp2.after_subj_nonpractice <- exp2.after_subj %>%
    filter(exp_condition != "practice")
exp2.n_rows_removed_subject_nonpractice <- nrow(exp2.no.practice) -
    nrow(exp2.after_subj_nonpractice)
...
exp2.p_removed_rt <- round(
    100 * exp2.n_removed_rt / nrow(exp2.after_subj_nonpractice),
    2
)

exp2.after_subj_rt_nonpractice <- exp2.after_subj_nonpractice %>%
    filter(RT < 4999 & RT > 200)
exp2.n_missing_trials <- exp2.after_subj_rt_nonpractice %>%
...
exp2.p_trials.removed_total <- round(
    100 * exp2.n_trials.removed_total / exp2.n_trials.raw,
    2
)

exp2.deletion <- round(
    100 *
        ((nrow(exp2.no.practice) - nrow(exp2.clean)) / nrow(exp2.no.practice)),
    2
)

exp2.missing_sentence <- ifelse(
    exp2.n_missing_trials == 0,
    "After RT trimming, there were no missing responses.",
    paste0(
        "After RT trimming, ",
        exp2.n_missing_trials,
        " trials had missing responses and were excluded."
    )
)
...
<!-- CHECK THIS: This intro paragraph is fine. Reads naturally. No major AI tells. -->
Experiment 1 showed no attraction from verbal attractors, but the mixed design---combining nominal and verbal conditions in the same session---raises the concern that the presence of robust nominal-attractor items may have influenced response patterns. Experiment 2 addressed this by removing all nominal-attractor conditions, testing whether the null verbal effect persists when verbal morphology is the sole potential source of interference.

## Participants

We recruited `r exp2.nsubj.raw` undergraduate students to participate in the experiment in exchange for course credit. All participants self-identified as native Turkish speakers, with an average age of `r exp2.meanage` years (range: `r exp2.minage`-`r exp2.maxage`).

Subject-level screening used one discrimination check. Because Experiment 2 contained only verbal attractor conditions, we computed each participant's difference in acceptance rates between grammatical (singular-verb) and ungrammatical (plural-verb) sentences with singular attractors. We excluded participants whose discrimination score fell at or below `r 100*exp2.acc.threshold`%. This removed `r exp2.n_bad` participant(s) (`r exp2.p_subj_excluded`% of the recruited sample). This subject-level filtering removed `r exp2.n_rows_removed_subject_nonpractice` rows before trial-level trimming.

At the trial level, response-time trimming removed `r exp2.n_removed_rt` trials (`r exp2.n_rt_fast` with RT <= 200 ms; `r exp2.n_rt_slow` with RT >= 4999 ms; `r exp2.p_removed_rt`% of trials). `r exp2.missing_sentence` In total, `r exp2.p_trials.removed_total`% of collected trials were excluded, and the final dataset contained `r exp2.nsubj.analysis` participants and `r exp2.n_trials.analysis` observations.

## Materials

We used the same verbal attractor items and fillers as in Experiment 1, removing all nominal attractor trials.
...
make_priors <- function(
    inter_ga_mean = 0.0,
    inter_ga_sd = 0.10, # Gram × Attr (classic attraction term)
    main_g_mean = 1.0,
    main_g_sd = 0.50, # Grammaticality main effect (your previous spec)
    main_a_mean = 0.30,
    main_a_sd = 0.40, # Attractor Number main effect
    intercept_mean = 0.85,
    intercept_sd = 0.70,
    exp_rate = 1,
    lkj_eta = 2
    ...
    c(
        ...
        # Main effects
        set_prior(
            sprintf("normal(%g, %g)", main_g_mean, main_g_sd),
            class = "b",
            coef = "grammaticalGram_minus_Ungram"
        ),
        set_prior(
            sprintf("normal(%g, %g)", main_a_mean, main_a_sd),
            class = "b",
            coef = "attractor_numPlural_minus_Singular"
        ),
        # Two-way interactions
        set_prior(
            sprintf("normal(%g, %g)", inter_ga_mean, inter_ga_sd),
            class = "b",
...
exp2_fit_paths <- c(
    "../../utility/models/m.exp2.rds",
    "../../utility/models/m.exp2.full.prior_n10.rds"
)
exp2_fit_available <- exp2_fit_paths[file.exists(exp2_fit_paths)]
if (length(exp2_fit_available) == 0) {
    stop(
        sprintf(
            "Missing fitted Exp2 model file. Checked: %s",
            paste(exp2_fit_paths, collapse = ", ")
        )
    )
}
m.exp2 <- readRDS(exp2_fit_available[[1]])
...
txt_p <- list(
    gram = fmt_prob(p_gt0(m.exp2, coef_names$gram), d = 2),
    attr = fmt_prob(p_gt0(m.exp2, coef_names$attr), d = 2),
    inter = fmt_prob(p_gt0(m.exp2, coef_names$inter), d = 2)
)
...
draws_exp2 <- as_draws_df(m.exp2)
b_attr <- get_col(draws_exp2, coef_names$attr)
b_inter <- get_col(draws_exp2, coef_names$inter)

# With Grammaticality coded as Grammatical = +0.5, Ungrammatical = -0.5:
# attractor effect within each grammaticality level is b_A +/- 0.5 * b_G:A
eff_attr_g <- b_attr + 0.5 * b_inter
eff_attr_u <- b_attr - 0.5 * b_inter
...
post_int_g <- list(
    attr_g = summ_draw(eff_attr_g),
    attr_u = summ_draw(eff_attr_u)
)
...
txt_g_p <- list(
    attr_g = fmt_prob(mean(eff_attr_g > 0), d = 2),
    attr_u = fmt_prob(mean(eff_attr_u > 0), d = 2)
)
...
On the other hand, accuracy in grammatical conditions was modulated by the number of the attractor in an unexpected way. Participants rated grammatical sentences with singular attractors as grammatical less often (`r get_value(exp2.avgs, text, grammatical == "grammatical", attractor_num == "singular")`) compared to their counterparts with plural attractors (`r get_value(exp2.avgs, text, grammatical == "grammatical", attractor_num == "plural")`).
...
<!-- CHECK THIS: Model spec paragraph is fine — technical and factual. No AI tells. -->
We fitted a Bayesian mixed-effects logistic regression to binary *yes/no* responses using brms, with the same model family (Bernoulli, logit link) and uninformative priors as in Experiment 1. Because Experiment 2 included only verbal attractor conditions, the model had a simpler 2 × 2 structure: fixed effects for Grammaticality and Attractor Number (both sum-coded at ±0.5) and their interaction, with maximal random intercepts and slopes for both subjects and items.

<!-- CHECK THIS: Results paragraph is okay — dense but results paragraphs are supposed to be. "crucially" is a minor AI tell but common in academic writing too. The final sentence ("the positive interaction is driven entirely by...") is a good interpretive closer. -->
Posterior estimates are summarized in @fig-exp2-fixed-effects. The model revealed a positive effect of grammaticality ($\beta$ = `r txt$gram`, P($\beta$ > `r txt_p$gram`)), but no reliable main effect of attractor number ($\beta$ = `r txt$attr`, P($\beta$ > `r txt_p$attr`)). The interaction was small but positive ($\beta$ = `r txt$inter`, P($\beta$ > `r txt_p$inter`)); crucially, this interaction reflected a grammatical-condition asymmetry rather than attraction. To clarify the pattern within each grammaticality level, we computed model-implied attractor-number effects from posterior draws. In grammatical conditions, plural attractors increased acceptance relative to singular attractors ($\beta$ = `r txt_g$attr_g`, P($\beta$ > 0) = `r txt_g_p$attr_g`). In ungrammatical conditions---the attraction-relevant comparison---there was no evidence for an attractor-number effect ($\beta$ = `r txt_g$attr_u`, P($\beta$ > 0) = `r txt_g_p$attr_u`). Thus, the presence of a plural verbal attractor did not increase the acceptability of ungrammatical sentences; the positive interaction is driven entirely by a grammatical-condition pattern that is opposite in direction to what an attraction account would predict.

```{r}
#| label: fig-exp2-fixed-effects
#| fig-cap: "Posterior means and 95% credible intervals for fixed effects in the Experiment 2 Bayesian model (log-odds scale)."
#| fig-width: 6
#| fig-height: 2
...
df_plot <- bind_rows(
    ...
    mutate(
        term_clean = case_when(
            ...
            term ==
                "b_grammaticalGram_minus_Ungram:attractor_numPlural_minus_Singular" ~ "Interaction",
            TRUE ~ term
        ),
        term_clean = factor(
            term_clean,
...
# Cache behavior:
# - If bf_exp2.rds exists, it is loaded.
# - If it does not exist, set bf_compute_if_missing_exp2 <- TRUE once to compute+save.
bf_cache_path_exp2 <- "../../utility/models/bf_exp2_bridge.rds"
bf_force_recompute_exp2 <- FALSE
bf_compute_if_missing_exp2 <- FALSE

compute_exp2_bf <- function(fit) {
    ...
    list(
        bf01_int = as.numeric(bf01_int),
        bf01_attr = as.numeric(bf01_attr)
    )
}
...
if (file.exists(bf_cache_path_exp2) && !bf_force_recompute_exp2) {
    bf_exp2 <- readRDS(bf_cache_path_exp2)
} else if (bf_force_recompute_exp2 || bf_compute_if_missing_exp2) {
    bf_exp2 <- compute_exp2_bf(m.exp2)
    dir.create(
        dirname(bf_cache_path_exp2),
        recursive = TRUE,
        showWarnings = FALSE
    )
    saveRDS(bf_exp2, bf_cache_path_exp2)
} else {
    bf_exp2 <- list(
        bf01_int = NA_real_,
        bf01_attr = NA_real_
...
bf01_int <- bf_exp2$bf01_int
bf01_attr <- bf_exp2$bf01_attr
bf01_int_txt <- format_bf(bf01_int)
bf01_attr_txt <- format_bf(bf01_attr)
...
<!-- CHECK THIS: Para 1 is fine — direct and factual. "This confirms that" is slightly AI but passes. -->
Experiment 2 replicated the verbal attractor conditions from Experiment 1 in isolation and again revealed no evidence for agreement attraction driven by verbal plural markers. Ungrammatical sentences with plural-marked reduced clause verbs were rejected at similar rates regardless of attractor number. This confirms that the absence of verbal attraction in Experiment 1 was not due to the presence of nominal attractor items or to the distributional properties of the mixed design.

<!-- CHECK THIS: Para 2 is mostly okay. "This effect is unlikely to reflect... since it arises" is slightly formulaic. "We do not pursue this explanation further, as it falls outside the scope" is very AI — consider "This lies beyond our present scope" or just cutting the sentence. The forward reference at the end is fine. -->
Unexpectedly, grammatical sentences with singular attractors were judged less acceptable than those with plural attractors. This effect is unlikely to reflect agreement attraction, since it arises in the opposite direction from what attraction would predict. One possibility is that it results from an interaction between plausibility and referential availability: the plural morpheme can license a more general interpretation, whereas the singular reduced relative clause more strongly invites a specific referent that may be less accessible in the task context. We do not pursue this explanation further, as it falls outside the scope of the present paper. We return to the theoretical implications of the converging null result across both experiments in the General Discussion.
